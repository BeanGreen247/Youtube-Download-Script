...
if [ $format -eq 1 ]
  ...
  read link
  /usr/local/bin/youtube-dl --hls-prefer-ffmpeg --cookies cookie.txt -i -f b --extract-audio --audio-format mp3 -o "%(title)s.%(ext)s" $link
...
if [ $format -eq 2 ]
  ...
  read link
  /usr/local/bin/youtube-dl --hls-prefer-ffmpeg --cookies cookie.txt -i --format mp4 -o "%(title)s.%(ext)s" $link
